class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/texture1.vert", "shaders/grey-sepia.frag"),
		...
		//Passing Uniform variables to the shaders
		...
		this.shadersList = {
			...
			'Sepia': 7,
			'Convolution': 8,
			'Yellow Up Blue Down':9,
			'Sepia (Grey-version)':10,
			'Water':11,
		};
	...
	update(t) {
		...
		if(this.selectedExampleShader == 11)
			this.testShaders[11].setUniformsValues({ timeFactor: t / 8000 % 1000 })
	}
